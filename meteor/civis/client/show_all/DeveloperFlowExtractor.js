function getDeveloper(graph){
	var codeChangeId = graph.nodes()[0];
	var codeChangeData = graph.node(codeChangeId);
	return codeChangeData.contributor;	
}

DeveloperFlowExtractor = function(aggregator){
...
DeveloperFlowExtractor.prototype.code_changes = function(graphs, developerName) {
	return graphs.filter(function(graph){
		return getDeveloper(graph) === developerName;
	});
};
...
DeveloperFlowExtractor.prototype.first_occurrence = function(graphs, developerName) {
	...
	for (var i = 0; i < graphs.length; i++) {
		if(getDeveloper(graphs[i]) === developerName){
